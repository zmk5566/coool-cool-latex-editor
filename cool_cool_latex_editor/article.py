...
SECTION_RE = re.compile(r"^\s*\\section\*?\{(?P<content>.*)\}\s*$")
...
def metadata_mask(source: str) -> str:
    masked = COMMENT_RE.sub(_mask_match, source)
...
def _block_id(kind: str, source: str, start: int, end: int) -> str:
    digest = hashlib.sha1(source[start:end].encode("utf-8")).hexdigest()[:10]
...
def parse_article(source: str) -> List[ArticleBlock]:
    ...
    for line in source[body_start:body_end].splitlines(keepends=True):
        ...
        cursor = line_end
    if cursor < body_end:
        line_records.append((cursor, body_end, body_end))
...
def article_block(source: str, block_id: str) -> ArticleBlock:
    for block in parse_article(source):
...
def update_article_block(
    source: str, block_id: str, segments: Iterable[Dict[str, object]]
) -> str:
    block = article_block(source, block_id)
